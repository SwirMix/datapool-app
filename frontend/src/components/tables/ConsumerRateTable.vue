...
export default {
  ...
  methods: {
    ...mapGetters(
        ['get_consumers_list', 'get_current_cache_info', 'get_consumers_chart', 'get_consumers_success_rate']
    ),
    ...mapActions(
        ['request_consumers_list']
    ),
    select_consumer(consumer){
        console.log(consumer)
        this.$router.push('/cache/' + this.get_current_cache_info().metadata.cacheMetadata.cache.publicCacheName + '/' + consumer)
    },
    get_consumers(){
        let body = {
            project: this.get_current_cache_info().metadata.cacheMetadata.cache.project,
            cache: this.get_current_cache_info().metadata.cacheMetadata.cache.cacheName,
            start: Moment(this.query.from.time).unix(),
            end: Moment(this.query.to.time).unix()
        }
        this.request_consumers_list(body)
    }
  },
  mounted() {
